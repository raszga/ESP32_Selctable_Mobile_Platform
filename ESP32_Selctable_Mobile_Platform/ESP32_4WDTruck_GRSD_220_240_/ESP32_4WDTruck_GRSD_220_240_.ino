...
IPAddress ip(192, 168, 5, 240);       // desired IP address
...
float K_Amp = 1.41;  // corection factor for amplitude
float K_RTT = 1;     /* Rotational coeficient - 
...
void loop() {
  ...
  ledcWrite(ledB, 0);                  //no packet
  if (packetSize) {
    ledcWrite(ledB, 255);         // packet received
    ...
    if (abs(RTT) < 50) { RTT = 0; }
    // special conditions
    //if (RTT != 0) { JY = -JY; }                             // reverse when RTT involved only for 4WD
    if ((JX != 0) | (JY != 0)) { RTT = int(RTT * K_RTT); }  // amplify RTT only
  ...
  |================= Rear  ===================|*/
    ...
    */
    ...
    int cmdMotLF = (V_Blue)-RTT;  // left front
    MotoMove(MotLF, -cmdMotLF, 50, 1);

    int cmdMotLR = (V_Red)-RTT;  //left rear
    MotoMove(MotLR, -cmdMotLR, 50, 1);

    Serial.print("      ");
    int cmdMotRF = (V_Red) + RTT;  // rigth front
    MotoMove(MotRF, -cmdMotRF, 50, 1);


    int cmdMotRR = (V_Blue) + RTT;  //rigth rear
